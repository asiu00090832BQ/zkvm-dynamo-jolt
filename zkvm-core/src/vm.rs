...
impl Zkvm {
    ...
    fn mul_lemma_6_1_1(a: u32, b: u32) -> u32 {
        ...
        let product =
            a_lo * b_lo +
            ((a_lo * b_hi + a_hi * b_lo) << 16) +
            ((a_hi * b_hi) << 32);
        product as u32
    }

    fn mulh_lemma_6_1_1(a: u32, b: u32) -> u32 {
        let a_lo = (a & 0xFFFF) as i64;
        let a_hi = ((a as i32) >> 16) as i64;
        let b_lo = (b & 0xFFFF) as i64;
        let b_hi = ((b as i32) >> 16) as i64;
        let product =
            a_lo * b_lo +
            ((a_lo * b_hi + a_hi * b_lo) << 16) +
            ((a_hi * b_hi) << 32);
    ...
    fn mulhsu_lemma_6_1_1(a: u32, b: u32) -> u32 {
        let a_lo = (a & 0xFFFF) as i64;
        let a_hi = ((a as i32) >> 16) as i64;
        let b_lo = (b & 0xFFFF) as u64 as i64;
        let b_hi = (b >> 16) as u64 as i64;
        let product =
            a_lo * b_lo +
            ((a_lo * b_hi + a_hi * b_lo) << 16) +
            ((a_hi * b_hi) << 32);
    ...
    fn mulhu_lemma_6_1_1(a: u32, b: u32) -> u32 {
        let a_lo = (a & 0xFFFF) as u64;
        let a_hi = (a >> 16) as u64;
        let b_lo = (b & 0xFFFF) as u64;
        let b_hi = (b >> 16) as u64;
    ...
    pub fn step(&mut self) -> Result<StepOutcome, ZkvmError> {
        ...
        let outcome = match instr {
            ...
            Instruction::Sub { rd, rs1, rs2 } => {
                self.write_reg(rd, self.regs[rs1].wrapping_add_sub(self.regs[rs2]));
                self.pc = next_pc;
                StepOutcome::Continue
            }
            ...
            Instruction::Mul { rd, rs1, rs2 } => {
                let val = Self::mul_lemma_6_1_1(self.regs[rs1], self.regs[rs2]);
                self.write_reg(rd, val);
                self.pc = next_pc;
                StepOutcome::Continue
            }
            Instruction::Mulh { rd, rs1, rs2 } => {
                let val = Self::mulh_lemma_6_1_1(self.regs[rs1], self.regs[rs2]);
                self.write_reg(rd, val);
                self.pc = next_pc;
                StepOutcome::Continue
            }
            Instruction::Mulhsu { rd, rs1, rs2 } => {
                let val = Self::mulhsu_lemma_6_1_1(self.regs[rs1], self.regs[rs2]);
                self.write_reg(rd, val);
                self.pc = next_pc;
                StepOutcome::Continue
            }
            Instruction::Mulhu { rd, rs1, rs2 } => {
                let val = Self::mulhu_lemma_6_1_1(self.regs[rs1], self.regs[rs2]);
                self.write_reg(rd, val);
                self.pc = next_pc;
                StepOutcome::Continue
            }
