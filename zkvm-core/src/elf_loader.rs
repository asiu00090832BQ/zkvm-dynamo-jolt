...
pub fn load_elf(bytes: &[u8]) -> Result<LoadedProgram, String> {
    ...
    // Invariant I1.3.1: Entry point MUST be 4-byte aligned
    if elf.entry % 4 != 0 {
    ...
    if elf.header.e_ident[4] != ELFCLASS32 || elf.header.e_ident[5] != ELFDATA2LSB || elf.header.e_machine != EM_RISCV {
        return Err("unsupported ELF: requires ELFCLASS32, Little-Endian, RISC-V".into());
    }
    ...
    for ph in &elf.program_headers {
        if ph.p_type == PT_LOAD {
            // Invariant I1.1: Alignment (Segment must be word-aligned)
            if ph.p_vaddr % 4 != 0 {
    ...
    // Invariant I1.3: Entry point residency in PF_X segment
    if !entry_in_exec {
    ...
    fn i in 0..segments.len().saturating_sub(1) {
    ...
    Ok(LoadedProgram { entry: elf.entry as u32, base, memory })
}
