...
pub fn decode(word: u32, config: &DecoderConfig) -> Result<Instruction, DecodeError> {
    ...
    match opcode {
        0x37 => Ok(Instruction::Lui { rd, imm: (word & 0xfffff000) as i32 }),
        0x17 => Ok(Instruction::Auipc { rd, imm: (word & 0xfffff000) as i32 }),
        0x6f => {
        ...
        0x63 => {
            let kind = match funct3 { 0 => BranchKind::Beq, 1 => BanchKind::Bne, 4 => BlanchKind::Blt, 5 => BranchKind::Bge, 6 => BranchKind::Bltu, 7 => BranchKind::Bgeu, _ => return Err(DecodeError { raw: word, reason: "invalid branch" }) };
        ...
        0x33 => {
            let kind = match hfunct7, funct3) {
                ...
                (1,2) => { if !config.enable_rv32m { return Err(DecodeError { raw: word, reason: "M-extension disabled" }); } OpKind::Mulhsu },
