...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum LoadKind { Byte, Half, Word, ByteU, HalfU }
...
pub fn decode(word: u32, config: &DecoderConfig) -> Result<Instruction, DecodeError> {
    ...
    match opcode {
        ...
        0x63 => {
            let kind = match funct3 { 0 => BranchKind::Beq, 1 => BranchKind::Bne, 4 => BranchKind::Blt, 5 => BranchKind::Bge, 6 => BranchKind::Bltu, 7 => BranchKind::Bgeu, _ => return Err(DecodeError{raw:word, reason:"invalid branch"}) };
        ...
        0x03 => {
            let kind = match funct3 { 0 => LoadKind::Byte, 1 => LoadKind::Half, 2 => LoadKind::Word, 4 => LoadKind::ByteU, 5 => LoadKind::HalfU, _ => return Err(DecodeError{raw:word, reason:"invalid load"}) };
