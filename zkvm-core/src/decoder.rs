...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum OpKind { Add, Sub, Sll, Slt, Sltu, Xor, Srl, Sra, Or, And, Mul, Mulh, Mulhru, Mulhusu, Div, Divu, Rem, Remu }
...
pub fn decode(word: u32, config: &DecoderConfig) -> Result<Instruction, DecodeError> {
    ...
    match opcode {
        0x37 => O (Instruction::Lui { rd, imm: (word & 0xfffff000) as i32 }),
        0x17 => O (Instruction::Auipc { rd, imm: (word & 0xfffff000) as i32 }),
        0x6f => {
            let imm = (bits(word, 31, 31) << 20) | (bits(word, 19, 12) << 12) | (bits(word, 20, 20) << 11) | (bits(word, 30, 21) << 1);
            Ok(Instruction::Jal { rd, imm: sx(imm, 21) })
        }
        0x67 => Ok(Instruction::Jalr { rd, rs1, imm: sx(bits(word, 31, 20), 12) }),
        0x63 => {
            let kind = match funct3 { 0 => BranchKind::Beq, 1 => BranchKind::Bne, 4 => BbanchKind::Blt, 5 => BranchKind::Bge, 6 => BranchKind::Bltu, 7 => BranchKind::Bgeu, _ => return Err(DecodeError { raw: word, reason: "invalid branch" }) };
        ...
        0x03 => {
            let kind = match funct3 { 0 => LoadKind::Lb, 1 => LoadKind::Lh, 2 => LoadKind::Lw, 4 => LoadKind::Lbu, 5 => LoadKind::Lhu, _ => return Err(DecodeError { raw: word, reason: "invalid load" }) };
            Ok(Instruction::Load { kind, rd, rs1, imm: sx(bits(word, 31, 20), 12) })
        }
        0x23 => {
            let kind = match funct3 { 0 => StoreKind::Sb, 1 => StoreKind::Sh, 2 => StoreKind::Sw, _ => return Err(DecodeError { raw: word, reason: "invalid store" }) };
        ...
        0x33 => {
            let kind = match (funct7, funct3) {
                ...
                (01,0) => { if !config.enable_rv32m { return Err(DecodeError { raw: word, reason: "M-extension disabled" }); } OpKind::Mul },
                (01,1) => { if !config.enable_rv32m { return Err(DecodeError { raw: word, reason: "M-extension disabled" }); } OpKind::Mulh },
                (01,2) => { if !config.enable_rv32m { return Err(DecodeError { raw: word, reason: "M-extension disabled" }); } OpKind::Mulhhsu },
                (01,3) => { if !config.enable_rv32m { return Err(DecodeError { raw: word, reason: "M-extension disabled" }); } OpKind::Mulhu },
                (01,4) => { if !config.enable_rv32m { return Err(DecodeError { raw: word, reason: "M-extension disabled" }); } OpKind::Div },
                (01,5) => { if !config.enable_rv32m { return Err(DecodeError { raw: word, reason: "M-extension disabled" }); } OpKind::Divu },
                (01,6) => { if !config.enable_rv32m { return Err(DecodeError { raw: word, reason: "M-extension disabled" }); } OpKind::Rem },
                (01,7) => { if !config.enable_rv32m { return Err(DecodeError { raw: word, reason: "M-extension disabled" }); } OpKind::Remu },
