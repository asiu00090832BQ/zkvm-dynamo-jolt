...
impl Decoder {
    pub fn decode(inst_word: u32) -> Result<Instruction, DecodeError> {
        ...
        match opcode {
            0x33 => match (funct7, funct3) {
                (0x00, 0x0) => Ok(Instruction::Add(Self::reg(rd)?, Self::reg(rs1)?, Self::reg(rs2)?)),
                (0x01, 0x0) => Ok(Instruction::Mul(Self::reg(rd)?, Self::reg(rs1)?, Self::reg(rs2)?)),
                (0x01, 0x1) => Ok(Instruction::Mulh(Self::reg(rd)?, Self::reg(rs1)?, Self::reg(rs2)?)),
                (0x01, 0x4) => Ok(Instruction::Div(Self::reg(rd)?, Self::reg(rs1)?, Self::reg(rs2)?)),
                (0x01, 0x6) => Ok(Instruction::Rem(Self::reg(rd)?, Self::reg(rs1)?, Self::reg(rs2)?)),
            ...
            0x63 => {
                let imm = (((inst_word >> 31) & 0x1) << 12) | (((inst_word >> 7) & 0x1) << 11) | (((inst_word >> 25) & 0x3F) << 5) | (((inst_word >> 8) & 0xF) << 1);
            ...
            0x6F => {
                let imm = (((inst_word >> 31) & 0x1) << 20) | (((inst_word >> 12) & 0xFF) << 12) | (((inst_word >> 20) & 0x1) << 11) | (((inst_word >> 21) & 0x3FF) << 1);
            ...
            0x37 => Ok(Instruction::Lui(Self::reg(rd)?, inst_word & 0xFFFF_F000)),
            0x17 => Ok(Instruction::Auipc(Self::reg(rd)?, inst_word & 0xFFFF_F000)),
    ...
    fn reg(n: u8) -> Result<Register, DecodeError> {
        match n {
            ...
            8 => Ok(Register::X8), 9 => Ok(Register::X9), 10 => Ok(Register::X10), 11 => Ok(Register::X11),
