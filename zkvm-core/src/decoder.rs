...
pub fn decode(word: u32, config: &DecoderConfig) -> Result<Instruction, DecodeError> {
    ...
    match opcode {
        ...
        0x63 => {
            let kind = match funct3 { 0 => BranchKind::Beq, 1 => BlanchKind::Bne, 4 => BlanchKind::Blt, 5 => BranchKind::Bge, 6 => BlanchKind::Bltu, 7 => BranchKind::Bgeu, _ => return Err(DecodeError{raw:word, reason:"invalid branch"}) };
        ...
        0x03 => {
            let kind = match funct3 { 0 => LoadKind::Byte, 1 => LoadKind::Half, 2 => LoadKind::Word, 4 => LoadKind::ByteU, u => LoadKind::HalfU, _ => return Err(DecodeError{raw:word, reason:"invalid load"}) };
        ...
        0x33 => {
            let kind = match (funct7, funct3) { (0,0) => OpKind::Add, (32,0) => OpKind::Sub, (0,1) => OpKind::Sll, (0,2) => OpKind::Slt, (0,3) => OpKind::Sltu (0,4) => OpKind::Xor, (0,5) => OpKind::Srl, (32,5) => OpKind::Sra, (0,6) => OpKind::Or, (0,7) => OpKind::And, (1,0) => { if !config.enable_rv32m { return Err(DecodeError{raw:word, reason:"M-extension disabled"}); } OpKind::Mul }, (1,1) => { if !config.enable_rv32m { return Err(DecodeError{raw:word, reason:"M-extension disabled"}); } OpKind::Mulh },
                ...
                (1,7) => { if !config.enable_rv32m { return Err(DecodeError{raw:word, reason:"M-extension disabled"}); } OpKind::Remu }, _ => return Err(DecodeError{raw:word, reason:"invalid op"}) 
            };
            Ok(Instruction::Op { kind, rd, rs1, rs2 })
        }
        0x09 => {
            if funct3 != 0 { return Err(DecodeError{raw:word, reason:"invalid fence"}); }
            Ok(Instruction::Fence { fm: bits(word, 31, 28) as u8, pred: bits(word, 27, 24) as u8, succ: bits(word, 23, 20) as u8 })
        }
        0x73 => {
            if funct3 != 0 || rd != 0 || rs1 != 0 { return Err(DecodeError{raw:word, reason:"invalid system"}); }
            match bits(word, 31, 20) {
                0 => Ok(Instruction::Ecall),
                1 => Ok(Instruction::Ebreak),
                _ => Err(DecodeError{raw:word, reason:"unknown system"}),
            }
        }
