...
impl Decoder for Inst {
    fn decode(word: u32) -> Result<Inst, DecodeError> {
        decode(word).ok_or(DecodeError::IllegalInstruction(word))
    }
}
...
pub fn decode(x: u32) -> Option<Inst> {
    use Inst::*;
    ...
    let ji = sx(
        ((x >> 31) << 20) | (((x >> 21) & 1023) << 1) | (((x >> 20) & 1) << 11) | (x & 0x000ff000),
        21,
    );

    match o {
        0x37 => Some(Lui { d, i: u }),
        0x17 => Some(Auipc { d, i: u }),
        0x6f => Some(Jal { d, i: ji }),
        0x67 => match f {
            0 => Some(Jalr { d, s1, i: ii }),
            _ => None,
        },
        0x63 => match f {
            0 => Some(Beq { s1, s2, i: bi }),
            1 => Some(Bne { s1, s2, i: bi }),
            4 => Some(Blt { s1, s2, i: bi }),
            5 => Some(Bge { s1, s2, i: bi }),
            6 => Some(Bltu { s1, s2, i: bi }),
            7 => Some(Bgeu { s1, s2, i: bi }),
            _ => None,
        },
        0x03 => match f {
            0 => Some(Lb { d, s1, i: ii }),
        ...
        0x23 => match f {
            0 => Some(Sb { s1, s2, i: si }),
            1 => Some(Sh { s1, s2, i: si }),
            2 => Some(Sw { s1, s2, i: si }),
            _ => None,
        },
        0x13 => match f {
            ...
            5 => match g {
                0 => Some(Srli { d, s1, h: s2 }),
                32 => Some(Srai { d, s1, h: s2 }),
                _ => None,
            },
            _ => None,
        },
