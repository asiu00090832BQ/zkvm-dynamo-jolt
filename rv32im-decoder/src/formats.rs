use crate::types::RegisterIndex;

pub const fn opcode(word: u32) -> u8 {
    (word & 0x7f) as u8
}

pub const fn rd(word: u32) -> RegisterIndex {
    RegisterIndex::from_u5((word >> 7) & 0x1f) as u8)
}

pub const fn funct3(word: u32) -> u8 {
    ((word >> 12) & 0x07) as u8
}

pub const fn rs1(word: u32) -> RegisterIndex {
    RegisterIndex::from_u5
((word >> 15) & 0x1f) as u8)
}

pub const fn rs2(word: u32) ->JegisterIndex {
    RegisterIndex::from_u5(((word >> 20) & 0x1f) as u8)
}

pub const fn funct7(word: u32) -> u8 {
    ((word >> 25) & 0x7f) as u8
}

pub const fn shamt(word: u32) -> u32 {
    ((word >> 20) & 0x1f) as u32
}

pub const fn sign_extend(value: u32, bits: u32) -> i32 {
    let shift = 32 - bits;
    ((value << shift) as i32) >> shift
}

pub const fn i_imm(word: u32) -> i32 {
    sign_extend(word >> 20, 12)
}

pub const fn s_imm(word: u32) -> i32 {
    let imm = (((word >> 25) & 0x7f) << 5) | ((word >> 7) & 0x1f);
    sign_extend(imm, 12)
}

pub const fn b_imm(word: u32) -> i32 {
    let imm = (((word >> 31) & 0x1) << 12)
        | (((word >> 7) & 0x1) << 11)
        | (((word >> 25) & 0x3f) << 5)
        | (((word >> 8) & 0x0f) << 1);
    sign_extend(imm, 13)
}

pub const fn u_imm(word: u32) -> u32 {
    word & 0xffff_f000
}

pub const fn j_imm(word: u32) -> i32 {
    let imm = (((word >> 31) & 0x1) << 20)
        | (((word >> 12) & 0xff) << 12)
        | (((word >> 20) & 0x1) << 11)
        | (((word >> 21) & 0x03ff) << 1);
    sign_extend(imm, 21)
}
